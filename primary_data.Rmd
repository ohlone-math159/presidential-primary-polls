...
Is there a difference for a particular candidate?
```{r}
...
```{r}
ggplot(data=primary_a4, mapping = aes(x=candidate_name, y=pct, color=candidate_name))+
     geom_boxplot()
```
Try to get the 2 on the same output
```{r}
ggplot(data=primary_a2, mapping = aes(x=candidate_name, y=pct, color=fte_grade))+
     geom_boxplot()
```
or view each grade on separate plot
```{r}
ggplot(data=primary_a5, mapping = aes(x=candidate_name, y=pct, color=candidate_name))+
    geom_boxplot()+
    coord_flip()+
...
timeline 
```{r}
ggplot(data=primary_a5, mapping = aes(x=poll_id, y=pct, color=candidate_name))+
    geom_bar()+
    facet_wrap(~ methodology, nrow=2 )
```
...
```{r}
ggplot(data=primary_a6, mapping = aes(x=poll_id, y=pct, color=candidate_name ))+
    geom_point()+
...
primary_a7 <- primary_a6 %>% 
  mutate(en_date1 = as.Date.factor(end_date, format = "%m/%d/%Y"))

```
next create new variable days_to_NH
need **lubridate** package
```{r}
NH_primary <- "2/11/19"
NH_primary <- as.Date.factor(NH_primary, format = "%m/%d/%Y")
